...
==============================================================================
 ...
 DSA OEL - Data Structures and Algorithms Lab
 University of Engineering and Technology, Lahore

 SYSTEM MENU (per DOCX specification):
   ...
   7. Algorithmic Performance Analysis
   8. Scalability Simulation
   9. System Reports
   0. Exit

 FILES USED (7 .txt files):
   Customers.txt         -> Customer profile data
   Riders.txt            -> Rider profile data
   Restaurants.txt       -> Restaurant profile data
   Orders.txt            -> All order records
   OrderHistory.txt      -> Status transition history log
   DeliveryRecords.txt   -> Completed delivery records
   KitchenLog.txt        -> Kitchen accept / prepare activity log

 DATA STRUCTURES (ALL CUSTOM - NO STL):
   Max-Heap            -> Order Priority Scheduling    O(log n)
   Circular Queue      -> Kitchen Load Balancing       O(1)
   Singly Linked List  -> Rider Pool Management        O(n)
   Adjacency Matrix + Dijkstra -> Route Optimization   O(V^2)
   Hash Table (Chaining)       -> Search & Retrieval   O(1) avg
   Stack               -> Order History / Undo         O(1)
    ...
    do {
        ...
        if (menuChoice == 1) {
            ...
            if (foodSystem->updateOrderStatusInFile(targetOrderId, "cancelled")) {
                ...
                cout << "\n\t[❌ ERROR]: Order ID Not Found in Database." << endl;
            }
            system("pause");
            ...
            cout << "\t[❌ INVALID CHOICE]: Please select a value between 0 and 6." << endl;
            system("pause");
        }
    } while (menuChoice != 0);
}
    ...
    do {
        ...
        if (menuChoice == 1) {
            ...
            if (sourceIdx == -1 || waypointIdx == -1 || destIdx == -1) {
                cout << "\t[❌ ERROR]: One or More Location Labels Not Found inside Map Graph Data." << endl;
            }
            else {
            ...
            cout << "\t[❌ INVALID CHOICE]: Please choose an option index between 0 and 4." << endl;
            system("pause");
        }
    } while (menuChoice != 0);
}
...
void searchRetrievalMenu() {
    ...
    do {
        ...
        cout << "==" << "====================================================================" << endl;
        cout << "||" << "\t 1.  Search by ID (O(1) Hash-Index Cross-Match)                 ||   " << endl;
        cout << "||" << "\t 2.  Filter Orders by Status (File Parse Strategy)              ||  " << endl;
        ...
        cout << "||" << "\t 7.  Filter Orders by Restaurant ID (Hash Table)                 ||" << endl;
        cout << "||" << "\t 8.  Filter Orders by Price Range (Hash Table)                   ||" << endl;
        cout << "||" << "\t 9.  Filter All Records by Keyword (Hash Table)                  ||" << endl;
        cout << "||" << "--------------------------------------------------------------------" << endl;
        cout << "||" << "\t 0.  Back to Main Menu                                    ||  " << endl;
        cout << "==" << "====================================================================" << endl;
        cout << "\tEnter Your Choice: ";
        ...
        if (menuChoice == 1) {
            ...
            cout << "==" << "====================================================================" << endl;
            cout << "||" << "\t\t\t O(1) HASH INDEX DATA DISCOVERY   " << endl;
            cout << "==" << "====================================================================" << endl;
...
void orderHistoryMenu() {
    ...
    do {
        ...
        if (menuChoice == 1) {
            ...
            ifstream orderReadFile(ordersFile);
            if (!orderReadFile.is_open()) {
                cout << "\n\t[❌ ERROR]: Failed to construct data stream channel from storage." << endl;
            }
            else {
                Order tempOrder;
                int displayCount = 0;
            ...
            cout << "\t[❌ INVALID CHOICE]: Please use an entry code index between 0 and 5." << endl;
            system("pause");
        }
    } while (menuChoice != 0);
}
...
void scalabilitySimulationMenu() {
    ...
    if (simulationCount <= 0 || simulationCount > 500) {
        cout << "\n\t[❌ ERROR]: Out of simulation bounds. Enter a value between 1 and 500." << endl;
        cout << "==" << "====================================================================" << endl;
        system("pause");
        return;
    }
    ...
    cout << "\n\t[⚡ RUNNING]: Constructing " << simulationCount << " simulated order nodes inside memory arrays..." << endl;
    ...
    cout << "\t[✅ COMPLETE]: Mapped " << simulationHeap.getSize() << " operations into dynamic structures." << endl;
    cout << "\n";
